...
const PostJob = () => {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const savedDraft = sessionStorage.getItem("postJobDraft");
      if (savedDraft) {
        try {
  ...
  const handleChange = (e: any) => {
    ...
    setFormData((prev) => {
      ...
      if (typeof window !== "undefined") {
        sessionStorage.setItem("postJobDraft", JSON.stringify(updated));
      }
      return updated;
    });
  };
  ...
  const handlesubmit = async (e: React.FormEvent) => {
    ...
    try {
      setisloading(true);
      try {
        await axios.post("https://elevance-skill.onrender.com/api/job", formData);
      } catch {
